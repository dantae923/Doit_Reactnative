...
export type AutoFocusContextType = {autoFocus: (event: FocusEvent) => void}
const defaultAutoFocusContext = {autoFocus: (event: FocusEvent) => {}}
...
export const AutoFocusProvider: FC<AutoFocusProviderProps> = ({
    ...
    const scrollToInput = useCallback((reactNode: any) => {scrollRef.current?.scrollToFocusedInput(reactNode)}, [])
    const autoFocus = useCallback((event: FocusEvent) => {scrollToInput(findNodeHandle)}, [])
