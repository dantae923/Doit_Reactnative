  ...
  const children = useMemo(() =>
    personInformations.map(({title, Component}:PersonInformation) => (
      <View style={{flex: 1}} key={title}>
        ...
        <FlatList data={people} renderItem={({item}) => <Component person={item}></Component>}
                  keyExtractor={(item, index) => item.id}
                  ItemSeparatorComponent={() => <View style={styles.itemSeparator}></View>}></FlatList>
      </View>
    ))
    , [people.length])
  return (
    <SafeAreaView style={styles.flex}>
      <TopBar setPeople={setPeople}></TopBar>
      <ScrollView horizontal contentContainerStyle={styles.horizontalScrollView}>{children}</ScrollView>
    </SafeAreaView>
  )
}
